...
function startUp(){
   document.getElementById("puzzle 1").innerHTML = "Puzzle 1";
   document.getElementById("puzzle").innerHTML =
      drawHitori(hitor11number,hitor11Rating,puzzle1block);
   var puzzleButtons = document.getElementsByClassName("puzzles");
   for (var i = 0; i < puzzleButtons.length; i++){
      puzzleButtons[i].onclick = whichPuzzle;
   }
}

var puzzleButtons = document.getElementsByClassName("puzzles");
   for (var i = 0; i < puzzleButtons.length; i++){
      puzzleButtons[i].onclick = findErrors;
   }

var puzzleButtons = document.getElementsByClassName("puzzles");
   for (var i = 0; i < puzzleButtons.length; i++){
      puzzleButtons[i].onclick = showSolution();
   }
...
function switchPuzzle(e){
   ...
   var puzzleID = e.target.id;
    switch (puzzleID) {
      case "puzzle1":
         document.getElementById("puzzle").innerHTML =
         drawPuzzle(hitor11number,hitor11Rating,puzzle1block);
         break;
      case "puzzle2":
         document.getElementById("puzzle").innerHTML =
         drawPuzzle(hitor11number,hitor11Rating,puzzle1block);
